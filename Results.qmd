...
In this section, we will present our analysis results using data from a study on volunteering [@cowles1987]. This data is available in the car data package [@carData-3].
...
```{r}
extrat <-t.test(extraversion ~ volunteer, data=Cowles)
neurot <- t.test(neuroticism ~ volunteer, data=Cowles)
```

Extraversion differed between those who volunteered, `r apa_print(extrat)$statistic`. As shown in @tbl-scoresbyvol, volunteers were more extraverted on average than those who did not volunteer, `r apa_print(extrat)$estimate`.

However, scores on neuroticism did not differ between groups, `r apa_print(neurot)$full_result`.
